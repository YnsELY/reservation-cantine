...
export default function OrdersPage() {
  ...
  const loadOrders = async () => {
    ...
    try {
      ...
      const parentIds = Array.from(new Set(reservations.map(r => r.parent_id).filter(Boolean)));
      ...
      const ordersList: OrderDetail[] = reservations.map((r: any) => {
  ...
  const filteredOrders = useMemo(() => {
    ...
    return orders.filter(o => {
      ...
      return name.includes(q) || o.parent_name.toLowerCase().includes(q);
    });
  }, [orders, searchQuery]);
    ...
    <SafeAreaView style={styles.container} edges={['top']}>
      <View style={styles.header}>
        ...
        <View style={styles.headerTitleBlock}>
          <Text style={styles.headerTitle} numberOfLines={1}>{menuName || 'Menu'}</Text>
      ...
      <ScrollView
        ...
        refreshControl={
          <RefreshControl refreshing={refreshing} onRefresh={() => { setRefreshing(true); void loadOrders(); }} />
        }
      >
        {filteredOrders.length === 0 ? (
          <View style={styles.emptyState}>
            ...
            <Text style={styles.emptyText}>
              {orders.length === 0 ? 'Aucune commande' : 'Aucune commande ne correspond à la recherche'}
            </Text>
          </View>
        ) : (
...
const styles = StyleSheet.create({
  ...
  modalScrollSection: { maxHeight: 200 },
  modalOption: {
    flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center',
